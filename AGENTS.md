...
- 覆盖 antd Collapse 的全局类时，选择器必须**比 antd 运行时注入的规则更具体**：antd 的 CSS-in-JS 在渲染时才注入 `<style>`（排在 `App.css` 之后），同优先级平局一律判给它。例如它会给 header 注入 `.ant-collapse > .ant-collapse-item > .ant-collapse-header { position: relative }`，所以 App.css 里冻结 header 的规则用重复类名（`.ant-collapse-header.ant-collapse-header`）拿到更高优先级，不要为了"简化"退回单类名——退回去 header 就不会冻结了（页面级工具栏是模块类名，不受影响）。同理 header 圆角也要压过 antd：单 item 卡片同时命中 `:first-child` 与 `:last-child`，后者的 `border-radius: 0 0 8px 8px` 胜出，App.css 用更高优先级显式把 header 圆角对齐卡片半径。优先用重复类名而不是 `!important`，因为 `!important` 会连"嵌套/弹层内 header 回到 `static`"的解冻规则一起压掉。
- 页面级工具栏冻结必须用 `position: sticky` 配对解冻，不要用 `position: fixed`。`--content-top-offset` 只作为 `main` 的 `padding-top`；sticky 的 `top` 相对这块已垫过的内容区是 `0`，不要再叠一次 offset，否则顶栏下方会空出一截。二级栏（筛选栏、搜索栏）要钉在外层栏正下方，offset 必须等于外层栏的**实测**高度，不能写死常量：外层栏高度由内容决定（换行、多语言、字号都会变），Gateway 用 `--gateway-header-height`（GatewayPage 用 ResizeObserver 发布）承载这个高度。所属 Collapse/页面滚出视口后必须自动解冻。全局 `.ant-collapse` 的 `overflow` 必须是 `clip`，不能是 `hidden` 也不能是 `visible`：`hidden` 让卡片变成滚动容器、sticky 直接失效（这就是工具栏会跟着滚走的原因）；`visible` 则丢掉卡片的圆角裁剪，而页面级卡片都是单 item（同时命中 antd 的 `:first-child` 与 `:last-child`，后者的 `border-radius: 0 0 8px 8px` 胜出），header 的不透明背景会在卡片圆角外画出直角。`clip` 只裁剪、不产生滚动容器。展开/收起动画的裁剪依赖 antd 打在 `.ant-collapse-panel` 上的 `.ant-motion-collapse { overflow: hidden }`，被卡片面板规则里的 `overflow: visible` 压掉后必须为 motion class 单独补回，否则正文会在动画期间溢出。嵌套 Collapse、Modal、Drawer 内的 header 必须保持 `position: static`，避免叠两层冻结栏。Skills/MCP 工具栏、会话搜索栏、Gateway/Image 顶栏、侧栏导航都走同一套规则。冻结栏只加 `position` / `top` / `z-index` / 不透明背景这四类声明，不要再加 `box-shadow: 0 1px 0 …` 分隔线或 `padding: 8px 0` 这类装饰：冻结不能改变页面原本的间距，也不能画出未冻结时不存在的边线（用户决定，`web/test/components/layout/stickyToolbar.test.ts` 守护）。
