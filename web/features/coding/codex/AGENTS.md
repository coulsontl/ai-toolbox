...
- Aggregate 保存重放前的拟议模型集必须与后端 catalog 判据一致：有效的 `modelCatalog.models[].model` 加 TOML 顶层 `model`；只有没有有效顶层 `model` 时才以 auto-review override 作为默认模型 fallback。不得把 `[chat].model` 纳入；损坏条目、非字符串或空 `model` 忽略。任一选中站点（正在编辑的 provider 除外）无法解析时，必须在 `restoreDirect`/保存前 fail closed。
...
- `requires_openai_auth` 由后端投影完全托管，前端**不得再默认注入**该行（issue #394）：默认模板（`useCodexConfigState` 的 `DEFAULT_CONFIG_TOML`）、`ensureCodexCustomProviderConfig`、它的字符串兜底模板、以及 All API Hub 导入模板都已移除。用户在供应商表单里用「requires_openai_auth」三态 `Select`（`settingsConfig.requiresOpenaiAuthMode` = `auto`/`keep`/`strip`，默认 `auto`）表达意图：`auto` **不写键**（absent 即 auto），只对 custom 生效，official 一律归一化为 auto。全局「切换第三方时保留官方登录」开启时该 Select **仍可选**，只把 `help` 换成 warning 文案（`requiresOpenaiAuthPreserveHint`）说明此时「自动」会移除该字段、需要账号能力（`/fast`、语音等）就选「始终写入 true」。**不要**写成 `disabled`：上游 `provider_uses_first_party_auth_path` 要求 `experimental_bearer_token.is_none()`，两个字段并存时凭据仍是 bearer 令牌、不存在 401，置灰会挡住 issue #394 这类「中转支持 OpenAI 鉴权令牌 + 要保留官方登录」的合法组合。优先级是 official > 显式 mode > preserve > 自动规则（后端 `project_codex_auth_to_runtime_config_with_mode`）。透传点必须**同步齐全**（漏任一处即静默丢回 auto，最容易漏的是每次模型目录编辑都会走的 `persistProviderCatalog`）：`buildCodexSettingsConfig`（含 `BuildCodexSettingsConfigInput`、`web/types/codex.ts` 的 `CodexSettingsConfig`）、`useCodexConfigState` 的 `parseInitialCodexState` / `CodexSettingsConfigSnapshot` / `resetFromSettingsConfig` / `getFinalSettingsConfig` / `resetCodexConfig`、`CodexPage.persistProviderCatalog`、以及两条 legacy 重建分支。
...
- 改 requires_openai_auth 三态或它的 settingsConfig 透传时，至少验证：表单选 keep/strip 保存后重新打开能回显；改一次模型目录（增删/排序）后 mode 不被重置回 auto；开启「保留官方登录」时控件仍可选、`help` 换成 warning 文案。跑 `pnpm test` 与 `pnpm exec tsc --noEmit`。
