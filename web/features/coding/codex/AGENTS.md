...
- 改 requires_openai_auth 三态或它的 settingsConfig 透传时，至少验证：表单选 keep/strip 保存后重新打开能回显；改一次模型目录（增删/排序）后 mode 不被重置回 auto；开启「保留官方登录」时控件置灰并显示说明文案。跑 `pnpm test` 与 `pnpm exec tsc --noEmit`。
