...
- 顶层 `baseUrl` 是唯一能跨模型列表增删改存活的渠道地址；`modelCatalog.models[].baseUrl` 只是它的 live 投影。`buildGrokProviderSettingsWithModels` 必须**保留**已有顶层值，并在缺失时按「所有非空条目 URL 一致」才从条目提升；反之删最后一个模型、或删掉唯一带 URL 的条目，会把整个渠道地址一起删掉（issue #391）。绝不改写其它条目的 per-model 值：混合 URL 的导入/第三方记录不是本应用能表达的形状，不能压平成渠道值，只能让存活条目保留各自地址。读取顺序固定为 顶层 `baseUrl` → 选中条目 → `settings.config` TOML 兜底；新模型与新导入条目必须继承渠道值。官方渠道不写顶层字段，重写官方记录时必须清掉泄漏的旧值。
