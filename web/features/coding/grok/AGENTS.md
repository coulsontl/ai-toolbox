...
- 表单「Base URL」是渠道级 SoT（模型映射 UI 不能改 per-model baseUrl），并且**落库在顶层 `settingsConfig.baseUrl`**。`buildGrokSettingsConfig` 在表单 Base URL 非空时必须同时写顶层字段并 **覆盖写入**全部 `modelCatalog.models[].baseUrl`；内置 endpoint 最终化也必须把顶层字段设为 `表单值 || endpoint 默认`，不能回退成 endpoint 默认。不要只在 catalog 缺 baseUrl 时补齐，否则编辑已保存供应商改 URL 后卡片与 live `[model.<key>].base_url` 仍残留旧值（issue #256）。
- 顶层 `baseUrl` 是唯一能跨模型列表增删改存活的渠道地址；`modelCatalog.models[].baseUrl` 只是它的 live 投影。`buildGrokProviderSettingsWithModels` 必须**保留**已有顶层值；缺失时只在「所有非空条目 URL 彼此一致」时把该地址提升为渠道值，否则删最后一个模型、或删掉唯一带 URL 的条目，会把整个渠道地址一起删掉（issue #391）。提升值来自条目本身，**绝不改写任何条目**：混合 per-model URL 的导入/第三方记录在「该次修改前仍存在两种以上非空地址」时完全不提升（条目各自保留地址，单次批量删空也不提升，不凭空造地址）；等到某次修改前只剩一种地址时，该地址会被提升并从此保留，因此渠道值始终与「当时的存活 catalog」自洽，代价是最终值跟随链式删除中最后剩下的那个地址。读取顺序固定为 顶层 `baseUrl` → 选中条目 → `settings.config` TOML 兜底；新模型与新导入条目必须继承渠道值。官方渠道不写顶层字段，重写官方记录时必须清掉泄漏的旧值。
- 清空模型列表后再新增/获取模型时，新条目必须继承**全部**渠道级字段：Base URL 来自顶层 `baseUrl`，API 格式来自 `resolveGrokChannelApiBackend`（选中条目 → `meta.apiFormat` → `settingsConfig` → 默认 `chat_completions`）。不要只读 `extractGrokSettingsApiBackend`：空 catalog 时它返回 undefined，新条目会写成缺 `api_backend` 的 `[model.<key>]`，CLI 回落 chat_completions 而卡片仍显示 Anthropic/Responses（issue #391 review）。已有条目自己的 `api_backend` 仍然优先，不被渠道值覆盖。
