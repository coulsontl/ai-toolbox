...
- V2 模式下，磁盘格式与 `OpenCodeConfig` 内部 V1 编辑结构由 `v2_migration` 在读取/写入边界双向映射。**provider/model 的落盘形状必须同时满足两条 OpenCode 线，不能只按开发线写**：已发布的 2.0.x 线（2026-09-24 在 2.0.15 真机实测，事实源是 `origin/v2` 的 `packages/core/src/provider.ts`、`model-resolver.ts`）读 provider 顶层 `package` + `settings`（base URL 必须放在 `settings.baseURL`）+ `headers`/`body`，model 读 `modelID`、可选 `package`（解析顺序是 `model.package ?? provider.package`）与 `settings`；只写 `api: {type:"aisdk", package, url}` + `request.headers/body`、缺顶层 `package` 时会以 `Unsupported package for <provider>/<model>:` 失败。开发线（`../opencode/packages/core/src/v1/config/migrate.ts`、`packages/core/src/config/provider.ts`）用的是 `api`/`request`，两线解码器都是 `onExcessProperty: "ignore"`，因此 `migrate_provider`/`migrate_model` 同时写两套键（flat 优先），并已在 2.0.15 上用 `@ai-sdk/openai-compatible` 与 `@ai-sdk/anthropic` 端到端验证（含 `modelID`、headers 覆盖）；`convert_*_v2_to_v1` 读入 flat 键后必须把它们从输出里删除，不能让两种形状互相回显。其余映射仍按 migrate.ts：model 用 `api.id` / `request` / `variants[].body` / `cost[]` / 完整 `capabilities`，agent 走 `agents`（`system`/`disabled`/`request.body`/`permissions`），MCP 走 `mcp.servers` 且 OAuth 嵌在 `oauth`、timeout 用 `{request}`；顶层还包含 `snapshot→snapshots`、`attachment→attachments`、`command→commands`、`skills` 数组、`reference→references`、`permission+tools→permissions`、`disabled/enabled_providers→experimental.policies`。不要退回 `mcp.timeout.{catalog,execution}` 这类旧实现形状。V2→V1 只负责把编辑器要维护的 `providers/plugins/agents/mcp/disabled_providers` 还原为 V1 视图，未知 V2 字段保留在 `other` 原样回写。所有常规保存都必须检查同一配置路径的 V1 备份标记并转回 V2；不能因页面仍使用 V1 数据结构而把活动配置降回 V1。用户关闭模式时恢复备份文件原始字节内容。回归：`cargo test --lib coding::open_code::v2_migration`。
