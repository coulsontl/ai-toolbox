...
pub async fn create_claude_provider_inner<R: tauri::Runtime>(
    state: &SqliteDbState,
    app: &tauri::AppHandle<R>,
    provider: ClaudeCodeProviderInput,
) -> Result<ClaudeCodeProvider, String> {
...
async fn apply_config_internal_with_events<R: tauri::Runtime>(
    ...
    record_provider_last_used_best_effort(db, "claudecode", provider_id);
...
fn record_provider_last_used_best_effort(
    db: &crate::db::SqliteDbState,
    module: &str,
    provider_id: &str,
) {
...
/// Disable the applied Claude Code prompt: clear every applied flag and empty
/// `CLAUDE.md`, while keeping the DB record so it can be re-applied later.
#[tauri::command]
pub async fn disable_claude_prompt_config(
    state: tauri::State<'_, SqliteDbState>,
    app: tauri::AppHandle,
    config_id: String,
) -> Result<(), String> {
    let db = state.db();
    get_claude_prompt_from_sqlite(db, &config_id)?
        .ok_or_else(|| format!("Prompt config '{}' not found", config_id))?;

    let now = Local::now().to_rfc3339();
    db.with_conn_mut(|conn| {
        db_update_applied_status(conn, DbTable::ClaudePromptConfig, None, &now)
    })?;
