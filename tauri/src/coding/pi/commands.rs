...
pub async fn read_pi_runtime_config(
    ...
    Ok(PiRuntimeConfig {
        ...
        settings_content: fs::read_to_string(&settings_path).ok(),
        auth_content: fs::read_to_string(&auth_path).ok(),
...
/// Disable the applied Pi prompt: clear every applied flag and empty the live
/// prompt file, while keeping the DB record so it can be re-applied later.
#[tauri::command]
pub async fn disable_pi_prompt_config(
    state: tauri::State<'_, SqliteDbState>,
    app: tauri::AppHandle,
    config_id: String,
) -> Result<(), String> {
    ...
    get_pi_prompt_from_sqlite(&db, &config_id)?
        .ok_or_else(|| format!("Prompt config '{}' not found", config_id))?;
    let now = Local::now().to_rfc3339();
    db.with_conn_mut(|conn| db_update_applied_status(conn, DbTable::PiPromptConfig, None, &now))?;
