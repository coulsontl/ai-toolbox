...
用户指出最近 30 天 DSH Session 有约 7.2 亿 Token，而成本为 0。前一次验收只核对了真实 Token、调用数与重复导入，临时库未复制生产价格表，也没有对真实模型断言非零估算金额。这是成本验收遗漏，不能将此前 Token 校验通过解释为费用也已正确。

| DSH 原生模型 | 调用数 | Token | 修复前金额 | 核对结论 |
|---|---:|---:|---:|---|
| `deepseek-v4-flash-0731` | 3,640 | 658,177,665 | 0 | 本地已有 `deepseek-v4-flash` 价格，原匹配器漏掉 MMDD 日期后缀 |
| `ox-alpha-free` | 209 | 65,780,492 | 0 | 原日志没有费用、本地没有该模型价格；不能仅凭名称判定免费 |

DSH 原始 usage 只有输入、输出、缓存读取计数，没有费用字段。V4 Flash 部分分别为新增输入 256,340,412、输出 2,370,334、缓存命中 399,466,919。沿用核对时本地价格表的每百万 Token 单价 `$0.14 / $0.28 / $0.0028`，按逐记录六位小数的现有持久化口径合计为 **$37.669858**，概览应显示 **$37.67**。这是可定价部分的估算，不是完整渠道账单。

修复同时处理新记录与旧归档：

- 公共模型匹配支持合法 `MMDD` 后缀，完整 ID（含显式零价）优先；非法月日和普通规格数字不能当作日期。
- Claude Code、Claude Desktop、DSH 保存原费用来源与金额。已有明确价格的零/正费用不随价格表变化重算；只补原来缺失的估算。
- 历史回填先用原生 ID、账本指纹及相容响应身份对照，再严格校验日/工具/模型分组的调用数、四类 Token、额外 Token 和旧金额。只有整组一致才更新费用，不重插原始记录，不修改调用数和 Token。
- 修复前后金额与账本同事务提交。Desktop 同步更新后续 audit 退回使用的贡献快照，防止已经补算的费用残留。
- 回填按工具隔离缓存，正常已定价会话的追加不能导致所有旧日志每分钟重扫。价格、文件集合或相关历史文件变化时重新核对；失败不写完成标记。
- 统计概览明确说明：未匹配定价的用量不计入金额，零显示不等于免费。

只读主库备份后，在副本执行修复，DSH 从 `$0.000000` 变为 **`$37.669858`**；3,849 次调用、723,958,157 Token 不变，修复了 3,640 条已归档贡献。将同一份价格表复制到全新临时库重新导入，金额完全一致；重复同步更新 0 条。

扩大核对也发现 Claude Code / Claude Desktop 存在同名模型的旧零成本汇总。公共匹配已修复，回填用例也覆盖这两个工具；但本机这些旧归档无法与当前原生日志整组对账，副本中保持原值，没有按新全文值强行改写已归档部分。该限制与此前 Desktop 早期快照归档的限制一致。

成本专项回归覆盖活跃/归档补价、精确零价、已知和未定价费用混合、不完整证据、事务失败重试、旧账本缺响应 ID、Desktop 补价后被 transcript 替代，以及无关日志更新不触发历史重扫。真实副本核对始终只写临时库，不执行付费模型请求。
